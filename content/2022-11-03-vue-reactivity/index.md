...
위 함수는 초기화 `defineReactive` 될 때 데이터 개체의 모든 속성에 대해 호출됩니다.

Observer getter가 종속성을 수집하도록 정의된 것을 볼 수 있으며 설정은 데이터 변경을 모니터링하고 변경이 감지되면 알림을 보냅니다.

### 위 메커니즘에서 두가지 문제가 발생함

- 속성의 삭제 or 추가를 감지할 수 없음
  - 반응성은 앱이 초기화될 때만 적용됩니다. 런타임에 새 속성을 추가하면 새 속성은 반응하지 않습니다. 즉, 속성 값을 변경해도 반응적인 부작용이 발생하지 않습니다. Vue2는 `.set` 을 개발자가 수동으로 속성을 반응형으로 추가할 수 있도록 하는 해결방법을 제공합니다.
- 성능
  - 대규모/ 중첩 데이터 셋의 경우 vue2가 모든 속성의 데이터 횡단에 getter/setter 생성이 필요하기 때문에 성능에 부정적인 영향을 미칠 수 있습니다.
...
proxy 개체를 사용하면 개체에 액세스하거나 개체를 수정하기 위한 모든 호출이 차단됩니다.
사용자 정의 작업은 getter / setter에서 정의됩니다.
...
속성이 존재하지 않는 경우(`hadKey` false인 경우) 트리거 메서드를 사용하여 새속성이 추가되었음을 종속성에 알립니다. 이것은 Vue2에서 추가 속성을 감지할 수 없는 문제를 해결합니다. 마찬가지로 `deleteProperty` 핸들러 작업은 Vue2에서 삭제 속성을 감지 할 수 없는 문제를 해결합니다.

Vue3에서 반응성 API의 프록시 구현을 사용하면 데이터의 모든 속성을 탐색할 필요가 없습니다. 이는 대규모 데이터 세트를 처리할 때 상당한 성능 향상을 가져옵니다.

## 결론

프록시는 강력한 메타프로그래밍 기능입니다. Vue3에서 Proxy를 적용하는 것은 훌륭한 솔루션입니다. Vue2의 문제를 해결할 뿐 아니라 추가 확장 가능성도 열어줍니다.
