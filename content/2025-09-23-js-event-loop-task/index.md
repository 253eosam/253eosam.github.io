...
> 한줄 정의: 이벤트 루프는 **매크로 태스크**와 **마이크로 태스크**라는 두 종류의 작업 큐를 운영하며, 마이크로 태스크가 항상 먼저 소진된 뒤 매크로 태스크가 하나씩 처리됩니다.
...
`setTimeout`의 딜레이가 0이므로 곧바로 실행될 것 같지만, 실제 출력은 다음과 같습니다.
...
Promise 콜백이 setTimeout 콜백보다 먼저 실행됩니다. 이 동작을 이해하려면 이벤트 루프가 관리하는 두 가지 작업 큐, **매크로 태스크(Macro Task)**와 **마이크로 태스크(Micro Task)**의 차이를 알아야 합니다.

## 매크로 태스크와 마이크로 태스크

### 매크로 태스크 (Macro Task)

브라우저나 Node.js 같은 호스트 환경이 제공하는 비동기 API가 생성하는 작업입니다. 이벤트 루프가 한 번 순회할 때 **하나씩** 꺼내서 처리합니다.
...
JavaScript 엔진 자체에서 생성하는 작업입니다. 매크로 태스크보다 우선순위가 높으며, 현재 실행 중인 코드가 끝나면 **큐가 완전히 빌 때까지** 연속 처리됩니다.
...
1. **콜 스택의 동기 코드**를 모두 실행합니다.
2. **마이크로 태스크 큐**가 빌 때까지 전부 처리합니다.
3. **매크로 태스크**를 하나 꺼내 실행합니다.
4. 다시 2번으로 돌아갑니다.
...
핵심은 7~8단계입니다. 매크로 태스크 실행 도중에 생성된 마이크로 태스크는, 다음 매크로 태스크로 넘어가기 전에 먼저 소진됩니다.

## async/await의 동작 방식

`async/await`는 Promise 기반 문법이므로 `await` 이후의 코드는 마이크로 태스크로 처리됩니다.
...
마이크로 태스크 큐가 빌 때까지 매크로 태스크로 넘어가지 않기 때문에, 마이크로 태스크가 자기 자신을 계속 등록하면 매크로 태스크는 영원히 실행되지 않습니다. 브라우저 탭이 멈춘 것처럼 보이게 됩니다.
...
복잡한 비동기 코드를 분석할 때 다음 순서로 추적하면 정확도를 높일 수 있습니다.

1. 동기 코드를 위에서 아래로 나열합니다.
2. 각 비동기 API가 어느 큐에 등록되는지 분류합니다 (매크로 / 마이크로).
3. 동기 코드 완료 후 마이크로 태스크 큐를 전부 소진합니다.
4. 매크로 태스크를 하나 실행한 뒤, 다시 마이크로 태스크 큐를 확인합니다.
5. 매크로 태스크 실행 중 새로 등록된 비동기 작업이 있으면 해당 큐에 추가합니다.

## Node.js 환경의 차이점

Node.js에는 `process.nextTick`이라는 별도의 마이크로 태스크가 존재합니다. 일반 마이크로 태스크(Promise)보다 우선순위가 높습니다.
...
사용자 인터랙션 핸들러에서 중요한 상태 업데이트와 무거운 후처리를 분리할 수 있습니다.
...
- 이벤트 루프는 **매크로 태스크 큐**와 **마이크로 태스크 큐**, 두 가지 작업 큐를 운영합니다.
- 실행 순서: **동기 코드 → 마이크로 태스크 전부 → 매크로 태스크 1개 → 마이크로 태스크 전부 → 반복**
- `Promise.then`, `async/await`, `queueMicrotask`는 마이크로 태스크입니다.
- `setTimeout`, `setInterval`, I/O, UI 이벤트는 매크로 태스크입니다.
- 매크로 태스크 실행 중 생성된 마이크로 태스크는 다음 매크로 태스크 전에 소진됩니다.
- Node.js의 `process.nextTick`은 일반 마이크로 태스크보다 우선순위가 높습니다.
- `.then()` 내부에서 Promise를 반환하면 추가 틱이 발생하여 실행 순서가 밀릴 수 있습니다.
