...
- [매크로 태스크와 마이크로 태스크](#매크로-태스크와-마이크로-태스크)
  - [매크로 태스크 (Macro Task)](#매크로-태스크-macro-task)
  - [마이크로 태스크 (Micro Task)](#마이크로-태스크-micro-task)
- [이벤트 루프의 실행 순서](#이벤트-루프의-실행-순서)
  - [기본 예제](#기본-예제)
  - [매크로 태스크 안에서 마이크로 태스크가 생성되는 경우](#매크로-태스크-안에서-마이크로-태스크가-생성되는-경우)
  - [실행 순서 분석 절차](#실행-순서-분석-절차)
- [Node.js 환경의 차이점](#nodejs-환경의-차이점)
- [async/await의 동작 방식](#asyncawait의-동작-방식)
- [흔한 함정](#흔한-함정)
  - [무한 마이크로 태스크](#무한-마이크로-태스크)
  - [Promise 체인에서 추가 틱이 발생하는 경우](#promise-체인에서-추가-틱이-발생하는-경우)
- [실무 활용](#실무-활용)
  - [작업 우선순위 분리](#작업-우선순위-분리)
  - [비동기 테스트에서 실행 순서 제어](#비동기-테스트에서-실행-순서-제어)
- [요약](#요약)
...
브라우저의 이벤트 루프와 구조는 같지만, Node.js에는 `process.nextTick`이라는 별도의 큐가 존재한다. 일반 마이크로 태스크(Promise)보다 우선순위가 높으며, Node.js 공식 문서에서도 별도의 "nextTick queue"로 구분한다.
...
Node.js의 우선순위는 `process.nextTick` > `Promise` 순서다. `setImmediate`는 이벤트 루프의 check 페이즈에서 실행되므로 `setTimeout(fn, 0)`과의 순서는 실행 시점에 따라 달라질 수 있다.

> **Q: `setImmediate`와 `setTimeout(fn, 0)`의 순서가 불확정적인 이유는?**
>
> `setTimeout(fn, 0)`은 실제로 최소 1ms의 딜레이를 갖는 경향이 있다. 이벤트 루프 진입 시점에 1ms가 이미 경과했으면 timer 페이즈에서 `setTimeout`이 먼저 실행되고, 아직 경과하지 않았으면 check 페이즈의 `setImmediate`가 먼저 실행되는 경우가 많다. 두 API가 I/O 콜백 내부에서 호출되면 `setImmediate`가 항상 먼저 실행된다.

## async/await의 동작 방식

`async/await`는 Promise 기반 문법이므로 `await` 이후의 코드는 마이크로 태스크로 처리된다. 앞서 살펴본 실행 순서 규칙이 그대로 적용된다.
...
>
> `await expr` 이후의 코드는 대략 `Promise.resolve(expr).then(이후코드)`와 유사하게 동작한다. 결국 `.then()` 콜백과 같은 마이크로 태스크 타이밍에 실행되는 것이다. 다만 엔진 최적화 수준에 따라 내부 틱 수는 다를 수 있다.
...
마이크로 태스크 큐가 빌 때까지 매크로 태스크로 넘어가지 않기 때문에, 마이크로 태스크가 자기 자신을 계속 등록하면 매크로 태스크는 영원히 실행되지 않는다. 브라우저 탭이 응답하지 않는 것처럼 동작하게 된다.
...
- Node.js의 `process.nextTick`은 일반 마이크로 태스크보다 우선순위가 높은 별도의 큐다.
