...
# Next.js + FSD + React Query 개발 가이드
...
- [기본 원칙](#기본-원칙)
- [한 화면이 만들어지는 흐름](#한-화면이-만들어지는-흐름)
- [추천 폴더 구조](#추천-폴더-구조)
- [shared: 공통 재료](#shared-공통-재료)
- [entities: 무엇을 다루는가](#entities-무엇을-다루는가)
...
- [features: 어떤 행동을 하는가](#features-어떤-행동을-하는가)
...
- [widgets: 화면 조합](#widgets-화면-조합)
- [transaction-list widget 예시](#transaction-list-widget-예시)
- [app: 라우팅과 조립](#app-라우팅과-조립)
- [지출 페이지 예시](#지출-페이지-예시)
- [수입 페이지 예시](#수입-페이지-예시)
- [새 목록 페이지를 추가할 때](#새-목록-페이지를-추가할-때)
- [새 feature를 추가할 때](#새-feature를-추가할-때)
- [새 entity UI를 추가할 때](#새-entity-ui를-추가할-때)
- [page section을 분리할 때](#page-section을-분리할-때)
- [React Query prefetch를 적용할 때](#react-query-prefetch를-적용할-때)
...
이 문서는 Next.js App Router, Feature-Sliced Design(FSD), React Query를 한 프로젝트에서 함께 사용할 때의 코드 배치 기준을 정리한다. 목표는 "파일을 어디에 둘 것인가"를 매번 감으로 판단하지 않도록, 반복해서 쓸 수 있는 기준을 만드는 것이다.
...
이 글은 그 애매함을 줄이기 위한 실전 기준이다. 수입/지출 내역 화면을 예시로 삼아, 조회는 어디에 두고 변경은 어디에 두는지, `app/[route]/_ui`와 `widgets`는 어떻게 구분하는지, query key는 어떻게 일관되게 관리하는지를 순서대로 설명한다.
...
앞부분의 [기본 원칙](#기본-원칙)과 [한 화면이 만들어지는 흐름](#한-화면이-만들어지는-흐름)은 전체 구조를 잡기 위한 기준이다. 중간의 `entities`, `features`, `widgets`, `app` 섹션은 각 레이어에 어떤 코드가 들어가는지 보여준다. 마지막의 [레이어 판단 체크리스트](#레이어-판단-체크리스트)는 실제 구현 중 파일 위치가 헷갈릴 때 빠르게 확인하는 용도다.
...
이 글의 예시는 `거래내역 목록 화면`을 기준으로 한다. 화면 하나가 만들어지는 흐름을 먼저 보면 각 레이어의 역할이 더 선명해진다.

```txt
app/expenses/page.tsx
→ 어떤 페이지인지 결정하고, 필요한 query를 server prefetch한다.

widgets/transaction-list
→ 검색어 상태, 목록 렌더링, 빈 상태, action 배치를 하나의 화면 블록으로 묶는다.

entities/transaction
→ transaction 타입, 조회 API, query key, query option, 표시 UI를 제공한다.

features/delete-transaction
→ 삭제 버튼, delete mutation, optimistic update, invalidate 처리를 담당한다.

shared
→ button, input, badge, fetch helper처럼 도메인 없는 재료를 제공한다.
```

중요한 점은 widget이 데이터를 화면에 연결할 수는 있지만, query key나 API endpoint를 직접 설계하지는 않는다는 것이다. widget은 `useTransactions()` 같은 entity hook을 사용해 화면을 조합하고, 삭제 같은 행동은 `DeleteTransactionButton` 같은 feature UI를 배치해서 연결한다.
...
`shared`에는 도메인 지식이 없는 공통 재료를 둔다. 버튼, 입력창, fetch helper처럼 어느 도메인에서도 그대로 쓸 수 있어야 한다.
...
도메인 이름이 들어가거나 비즈니스 규칙이 포함되면 `shared`가 아니다. 예를 들어 `Badge`는 공통 재료지만, `TransactionBadge`는 거래내역이라는 도메인 의미를 가지므로 entity에 둔다.
...
entity의 중심은 조회다. 생성, 수정, 삭제처럼 사용자의 행동으로 서버 상태를 바꾸는 코드는 feature로 분리한다.
...
### feature UI에서 mutation을 연결하는 방식

`DeleteTransactionButton`, `CreateTransactionForm`, `FavoriteTransactionButton`처럼 특정 행동을 나타내는 feature UI는 보통 자기 feature의 mutation hook을 내부에서 호출한다.
...
즉 widget이 `mutate`를 만들어 feature UI에 props로 넘기는 구조가 기본은 아니다. widget은 삭제 버튼이 필요한 위치에 `DeleteTransactionButton`을 조합하고, 삭제라는 행동의 구현은 `features/delete-transaction` 안에서 닫아둔다. 이렇게 하면 widget은 화면 조합에 집중하고, feature는 행동의 세부 구현을 책임진다.

다만 모든 UI가 hook을 직접 호출해야 하는 것은 아니다. `ConfirmDialog`처럼 삭제 전용이 아닌 범용 UI는 `onConfirm`, `isPending` 같은 props를 받는 편이 자연스럽다.
...
화면용 상태 관리
검색 결과 필터링 연결
route에서 분리해도 의미가 유지되는 화면 블록 제공
```
...
여기서 "직접 API 호출 금지"는 widget이 `fetch()`나 `getTransactions()`를 직접 호출하지 말라는 뜻이다. `useTransactions(type)`처럼 entity가 제공하는 query hook을 사용하는 것은 괜찮다. widget은 entity hook과 feature UI를 가져와 화면을 조합하는 위치다.
...
이 문서에서는 수입/지출처럼 구조가 유사한 도메인을 추상화하는 예시로 `transaction`을 사용한다.
