...
백엔드 API 서버가 발급하는 일반적인 JWT(서명됨)입니다. 프론트엔드에서는 서명 검증을 하지 않고 payload만 읽습니다.
...
NextAuth가 구성하는 보안은 5개의 계층으로 이루어져 있습니다.
...
- **JWT 검증**은 서명 검증(위변조 확인)과 클레임 검증(만료/발급자 확인)으로 나뉩니다.
  프론트엔드는 백엔드 JWT의 서명을 검증하지 않고 payload만 참고용으로 읽습니다.
