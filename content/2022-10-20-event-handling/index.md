...
이벤트 관련 이슈를 작업 도중 자꾸 헷갈리는 이벤트 수식어와 핸들 과정을 정리해보는 시간을 가져보겠습니다.

브라우저에서 이벤트가 어떻게 감지되는지 또 어떻게 전파되는지 확인해봅니다.

## 이벤트 버블링 - Event Bubbling

이벤트 버블링은 특정 화면 요소에서 이벤트가 발생하였을때 해당 이벤트가 더 상위의 요소들로 전달되는 특징을 말합니다.
기본적인 옵션으로 적용되어있습니다.

![](https://velog.velcdn.com/images/253eosam/post/9775b9df-8ae7-41c2-825f-aac53ada52c8/image.png)

> Tip. HTML은 기본적으로 트리 구조를 갖고있습니다.

위 지식을 알고있는 사람들은 대충 무슨 말인지 이해하고있을것입니다.
하지만 나도 브라우저의 이벤트 동작 원리를 공부하지 않았을땐 반대의 개념으로 이해하고있던터라 상당히 충격이였습니다.
자세한 내용은 아래 코드를 보면서 이해해보면 좋을 것입니다.

![](https://velog.velcdn.com/images/253eosam/post/f1bb84f1-3578-4811-ae42-af8410d84111/image.png)

초록색 영역을 한번 클릭시 콘솔창에는 `green` > `blue` > `red` 순으로 찍히는 것을 불 수있습니다. <br>
이것을 통해서 알수있는 것은 **왜 한번의 클릭으로 3개의 이벤트가 발생하였는가?**입니다. <br>
그 이유는 브라우저가 이벤트를 감지하는 방식에 있습니다.

브라우저는 특정 화면 요소에서 이벤트가 발생했을 때 그 이벤트를 최상위에 있는 화면 요소까지 이벤트를 전파시킵니다.
따라서 위와 같은 결과를 얻은것입니다. 여기서 `blue` 영역을 클릭했다면 `blue` > `red` 의 출력을 볼 수 있을 것입니다.

이러한 메커니즘을 이해한다면 자식 요소를 제어하다 부모요소에 등록된 이벤트가 실행되는 현상을 이해할 수 있을 것입니다.
...
위 방식은 버블링과는 반대되는 개념인데 최상위 요소인 body 태그에서 해당 태그를 찾아 내려갑니다.

![](https://velog.velcdn.com/images/253eosam/post/9f675120-032d-45c5-a397-5e2b06f0b619/image.png)

캡쳐링은 이벤트 옵션을 통해 설정할 수 있습니다.

위 코드를 입력 후 초록 여역을 클릭한다면 `red` > `blue` > `green` 순으로 출력되는 것을 확인할 수 있습니다.
