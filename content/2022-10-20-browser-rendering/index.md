...
- UI 백엔드
  ...
  - 플랫폼에서 명시하지 않은 일반적인 인터페이스로써 OS 사용자 인터페이스 체계를 사용합니다.
- 데이터 저장소
  - 웹에서 데이터를 하드디스크에 저장합니다.
...
브라우저는 `파싱` ->  `스타일` -> `레이아웃` -> `페인트` -> `합성` -> `렌더`등의 과정을 거칩니다. 그 후에 JS나 CSS를 통해 DOM이나 CSS에 변화가 생길 경우 reflow/ repaint 등의 과정을 수행합니다.

### 1. 파싱

#### DOM Tree 생성

HTML을 파싱하여 루트가 Document인 DOM 객체 트리를 구성합니다.
...
DOM tree,, CSSOM Tree가 생성되면 스타일을 매칭시켜주는 과정을 거쳐 렌더 트리를 구성합니다.  
렌더링 트리에는 **페이지를 렌더링하는데 필요한 노드만 포함**됩니다.
...
위 **Critical Render Path** 시간을 줄이면 브라우저가 웹 페이지를 보여주는 시간을 줄일수있습니다.

_<https://csstriggers.com> 해당 링크를 통해 CSS가 렌더링시 어떤 동작이 발생하는지 확인가능하고 이를 잘 활용한다면 최적화가 가능합니다._
...
레이어는 페인팅할 영역을 나누어 놓은 것을 의미합니다. 크롬의 경우 Render tree의 노드를 영역을 나누워 관리하고 각각 페인팅한 후 하나의 비트맵으로 합성하여 페이지를 완성합니다.
