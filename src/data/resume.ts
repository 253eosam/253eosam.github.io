  ...
  "experience": [
    {
      ...
      "projects": [
          ...
          "issues": [
            {
              ...
              "strategy": "디자인 산출물을 기준으로 UI framework와 Tailwind 설정을 먼저 맞추고 Storybook 기반 컴포넌트 체계를 선구축했습니다.",
              "execution": "UI framework + Tailwind 기반 디자인 시스템과 Storybook을 구축했습니다.",
              "impact": "사용자 체감 성능 기준 LCP 2.67s→1.68s, Speed Index 2.60s→1.16s, TBT 35.1ms→13.9ms로 개선했습니다.",
              "retrospective": "마이그레이션은 화면 재현보다 기존 비즈니스 플로우 해석과 우선순위 설계가 더 중요하다는 점을 확인했습니다."
            },
            {
              "problem": "오픈 이후 운영까지 고려하면 배포 방식, API 정합성, 초기 성능을 프로젝트 초반에 함께 정리해야 장기 유지보수가 가능한 상황이었습니다.",
              "strategy": "ASP 서비스 흐름을 분석한 뒤 전시 → 주문 → 결제 순으로 핵심 사용자 여정을 나눠 마이그레이션했습니다.",
              "execution": "PC/Tablet/Mobile 반응형 퍼블리싱과 ASP 서비스 구조 분석 후 페이지 마이그레이션을 진행했습니다.",
              "impact": "초기 스크립트 전송량 약 65%, 미사용 JavaScript 약 76%를 절감했습니다.",
        ...
        },
        {
          ...
          "issues": [
            {
              "problem": "전시 페이지와 이벤트성 모집 페이지는 목적과 트래픽 특성이 달라, 동일한 방식으로 운영하면 서버 부하와 사용자 체감 속도 측면에서 비효율이 발생할 수 있었습니다.",
              "strategy": "페이지 목적에 따라 SEO 확보와 서버 부하 완화의 우선순위를 다르게 두고, SSR/CSR 렌더링 전략을 분리했습니다.",
              "execution": "전시·성적 조회 등 서버 렌더링이 필요한 화면은 SSR로 유지하고, 모집·이벤트 페이지는 CSR + S3 기반으로 구성했습니다.",
              "impact": "동시 접속 16,000명 규모의 모집 페이지를 안정적으로 운영하며 서버 리소스 부담을 낮췄습니다.",
              "retrospective": "트래픽 대응은 기능 완성보다 병목 지점을 먼저 분리하는 설계가 핵심이라는 점을 체감했습니다."
            },
            {
              ...
              "impact": "페이지 특성별 렌더링 전략을 정리해 운영 중 신규 페이지 개발과 수정 시 판단 기준을 명확히 만들었습니다.",
              "retrospective": "초기에 렌더링 전략 기준을 더 명확히 문서화했다면 운영 중 의사결정 비용을 더 줄일 수 있었겠다고 판단했습니다."
            },
            {
              "problem": "선착순 모집 페이지는 짧은 시간에 트래픽이 집중되는 구조라, 일반적인 방식으로 처리하면 서버 병목과 사용자 대기 경험 저하가 발생할 수 있었습니다.",
              "strategy": "트래픽이 집중되는 상황에서도 서버 부담을 줄이고, 사용자가 대기 상태를 명확히 인지할 수 있도록 캐싱 전략과 로딩 UX를 함께 설계했습니다.",
              "execution": "선착순 모집 페이지에 TTL 기반 캐싱을 적용하고, 실제 응답 대기 구간에 맞춘 로딩 화면을 구현했습니다.",
              "impact": "공통 UI와 반응형 유틸리티를 정비해 이후 화면 유지보수 속도와 일관성을 개선했습니다."
            },
            {
              "problem": "진행 중이던 프로젝트를 충분한 인수인계 없이 신규 입사자 2명이 이어받아, 제한된 정보 속에서도 서비스를 안정적으로 오픈하고 운영할 수 있는 협업 기준이 필요했습니다.",
              "strategy": "빠른 운영 대응과 협업 일관성을 위해 코드 품질 규칙과 커밋 규칙을 표준화했습니다.",
              "execution": {
