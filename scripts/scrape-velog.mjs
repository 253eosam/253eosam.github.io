 ...
 * Velog 포스트 스크래핑 스크립트
 *
 * Velog GraphQL API(v2)를 사용하여 모든 포스트를 가져와
...
async function fetchAllPosts() {
  ...
  while (true) {
    const cursorArg = cursor ? `, cursor: "${escapeGql(cursor)}"` : '';
    const query = `{ posts(username: "${escapeGql(USERNAME)}"${cursorArg}) { id title url_slug released_at short_description tags } }`;
...
async function fetchPostDetail(urlSlug) {
  const query = `{ post(username: "${escapeGql(USERNAME)}", url_slug: "${escapeGql(urlSlug)}") { id title body released_at tags short_description } }`;
  const data = await graphqlFetch(query);
  return data.post;
}
